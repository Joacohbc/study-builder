...
const QuizTab = ({ quizData, onQuizComplete, activeSetName }) => { // Added activeSetName prop for context
    ...
    // State to store calculated results and feedback
    ...
    const handleSubmit = (event) => {
        ...
        currentQuestions.forEach(qData => {
            ...
            const userAnswer = answers[questionId]; // Get user's answer for this question
            let isCorrect = false;
            let questionFeedback = { isAnswered: false }; // Initialize feedback object

            try { // Wrap evaluation in try-catch for robustness
                ...
                if (qData.type === 'single') {
                    ...
                    (qData.definitions || []).forEach(definition => {
                        ...
                        const correctTerm = Object.keys(correctMatches).find(key => correctMatches[key] === definition) || null;
                         if (droppedTerm) {
                            answeredMatchCount++;
                ...
                questionFeedback.isCorrect = isCorrect;
                 ...
                 console.error(`Error evaluating question ${questionId}:`, error, qData);
                 questionFeedback.error = "Error al evaluar esta pregunta.";
                 questionFeedback.isCorrect = false;
            }
            detailedFeedback[questionId] = questionFeedback;
        });
                ...
                <form onSubmit={handleSubmit}>
                    <div id="quiz-container" className="space-y-8">
                        ...
                        {shuffledQuestions.map((qData, index) => {
                             // Find the detailed feedback for this question after submission
                             const questionFeedback = results?.feedback?.[qData.id];
