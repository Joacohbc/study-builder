...
const SingleChoiceQuestion = ({ questionData, questionIndex, selectedAnswer, onChange, isSubmitted, feedback, isIndividuallyChecked }) => { // Added isIndividuallyChecked
    ...
    return (<div className="space-y-3"> {shuffledOptions.map((option, index) => {
        const inputId = `q${questionIndex}_option${index}`; let labelClass = 'flex items-center p-3 border rounded-md hover:bg-gray-50 transition duration-150 cursor-pointer'; let inputClass = `mr-3 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 ${isSubmitted ? 'cursor-not-allowed' : ''}`; // isSubmitted controls disabled state
